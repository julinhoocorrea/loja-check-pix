  ...
  persist(
    (set, get) => ({
      ...
      estoque: {
        ...
        currentStock: mockEstoqueItems.reduce(
      ...
      updateRevendedor: (id, data) => {
        set((state) => ({
          revendedores: state.revendedores.map((revendedor) =>
            revendedor.id === id
              ? { ...revendedor, ...data, updatedAt: new Date() }
              : revendedor,
          ),
        }));
      },
      ...
      removeRevendedor: (id) => {
        set((state) => ({
          revendedores: state.revendedores.filter(
            (revendedor) => revendedor.id !== id,
          ),
        }));
      },
      ...
      processPaymentWebhook: (webhook) => {
        ...
        const payment = state.pendingPayments.find(
          (p: PendingPayment) => p.id === webhook.paymentId,
        );
      ...
      createVendaFromPayment: (paymentId) => {
        const state = get();
        const payment = state.pendingPayments.find(
          (p: PendingPayment) => p.id === paymentId,
        );
        if (!payment) return false;

        let onlineRevendedor = state.revendedores.find(
          (r: Revendedor) => r.name === "Vendas Online",
        );
        if (!onlineRevendedor) {
          state.addRevendedor({
            name: "Vendas Online",
          ...
          onlineRevendedor = state.revendedores.find(
            (r: Revendedor) => r.name === "Vendas Online",
          );
      ...
      checkPaymentStatus: async (paymentId) => {
        try {
          const response = await fetch(
            `https://api.best4send.com/p/v1/links/${paymentId}`,
            {
              headers: {
      ...
      getDashboardMetrics: () => {
        ...
        const receitaBruta = vendas.reduce(
          (total, venda) => total + venda.totalValue,
          0,
        );
        const lucroLiquido = vendas.reduce(
          (total, venda) => total + venda.netProfit,
          0,
        );
        const totalVendas = vendas.length;
        const revendedoresAtivos = revendedores.filter(
          (r) => r.isActive,
        ).length;
