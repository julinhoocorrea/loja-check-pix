...
import {
  AnaSearchService,
  type SearchResult,
} from "@/services/anaSearchService";
...
const configSchema = z.object({
  ...
  paymentInstructions: z
    .string()
    .min(1, "Instruções de pagamento são obrigatórias"),
  ...
  >([
    {
      type: "ana",
      message:
        "🤖 Olá! Eu sou a Ana, sua assistente inteligente. Posso ajudar você a buscar informações sobre vendas, revendedores, estoque, clientes e relatórios. Como posso te ajudar hoje?",
      timestamp: new Date(),
    },
  ]);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const errorMessage = {
        type: "ana" as const,
        message:
          "😅 Desculpe, houve um erro ao processar sua solicitação. Pode tentar novamente com outras palavras?",
        timestamp: new Date(),
      };
                  ...
                  {result.data.slice(0, 5).map((venda: any) => (
                    <TableRow key={venda.id}>
                      ...
                      <TableCell>
                        <Badge
                          variant={
                            venda.status === "pago" ? "default" : "secondary"
                          }
                        >
                          {venda.status}
                        </Badge>
                    ...
                    <div>
                      ...
                      {revendedor.performance && (
                        <div className="text-sm text-green-600">
                          {revendedor.performance.totalVendas} vendas •{" "}
                          {formatCurrency(revendedor.performance.receita)}
                        </div>
                      )}
                    </div>
                    <Badge
                      variant={revendedor.isActive ? "default" : "secondary"}
                    >
                  ...
                  >
                    <div>
                      ...
                      <div className="text-sm text-orange-600">
                        ...
                        {cliente.diamantesTotal.toLocaleString()} diamantes
                      </div>
                    </div>
  ...
  const features = [
    {
      ...
      description:
        "Responde automaticamente perguntas frequentes sobre diamantes e pedidos",
    },
    {
      icon: Zap,
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <div
              className={`w-2 h-2 rounded-full ${isConnected ? "bg-green-400" : "bg-slate-400"}`}
            />
            ...
            <CardContent>
              ...
              {isConfiguring && (
                <Alert className="mt-4">
                  ...
                  <AlertDescription>
                    Configurando IA Ana... Aguarde enquanto estabelecemos a
              ...
              {isConnected && (
                <Alert className="mt-4 border-green-200 bg-green-50">
                  ...
                  <AlertDescription className="text-green-800">
                    IA Ana está conectada e funcionando! Seus clientes já podem
                    conversar com ela no WhatsApp.
                  </AlertDescription>
                </Alert>
              )}
            </CardContent>
