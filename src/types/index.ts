...
export interface DataState {
  ...
  addRevendedor: (
    revendedor: Omit<
      Revendedor,
      "id" | "balance" | "totalSales" | "isActive" | "createdAt" | "updatedAt"
    > & { permissions?: UserPermissions },
  ) => void;
  ...
  // Venda Actions
  addVenda: (
    venda: Omit<Venda, "id" | "cost" | "profit" | "commission" | "netProfit">,
  ) => void;
  updateVenda: (id: string, data: Partial<Venda>) => void;
  updateVendaStatus: (id: string, status: Venda["status"]) => void;
  updateDeliveryStatus: (
    id: string,
    deliveryStatus: Venda["deliveryStatus"],
    userId: string,
  ) => void;

  // Estoque Actions
  addEstoqueItem: (
    item: Omit<EstoqueItem, "id" | "createdAt" | "updatedAt">,
  ) => void;
  updateEstoqueItem: (id: string, data: Partial<EstoqueItem>) => void;
  removeEstoqueItem: (id: string) => void;
  addMovimentacao: (
    movimentacao: Omit<EstoqueMovimentacao, "id" | "date">,
  ) => void;
  updateItemQuantity: (
    itemId: string,
    quantity: number,
    type: EstoqueMovimentacao["type"],
    reason: string,
  ) => void;

  // Payment Actions
  addPendingPayment: (
    payment: Omit<PendingPayment, "id" | "createdAt">,
  ) => void;
  updatePaymentStatus: (
    paymentId: string,
  ...
  processPaymentWebhook: (
    webhook: PaymentWebhook & { paymentId: string },
  ) => boolean;
