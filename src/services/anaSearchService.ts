import type { EstoqueItem, Revendedor, Venda } from "@/types";
...
export class AnaSearchService {
    ...
    for (const [intentType, intentPatterns] of Object.entries(patterns)) {
      for (const pattern of intentPatterns) {
        ...
        if (match) {
          ...
          if (
            normalizedQuery.includes("mês") ||
            normalizedQuery.includes("mes")
          )
            parameters.periodo = "mes";
  ...
  private searchVendas(
    ...
    const totalDiamantes = filteredVendas.reduce(
      (sum, v) => sum + v.diamondQuantity,
      0,
    );
  ...
  private searchRevendedores(
    ...
    // Filtrar por status
    if (
      query.parameters.target === "ativo" ||
      query.parameters.filter === "ativo"
    ) {
      filteredRevendedores = revendedores.filter((r) => r.isActive);
      summary = "Revendedores ativos";
    } else if (
      query.parameters.target === "inativo" ||
      query.parameters.filter === "inativo"
    ) {
      filteredRevendedores = revendedores.filter((r) => !r.isActive);
    ...
    if (
      query.parameters.target &&
      !["ativo", "inativo"].includes(query.parameters.target)
    ) {
    ...
    const revendedoresComPerformance = filteredRevendedores.map(
      (revendedor) => {
        const vendasRevendedor = vendas.filter(
          (v) => v.revendedorId === revendedor.id,
        );
        const totalVendas = vendasRevendedor.length;
        const receita = vendasRevendedor.reduce(
          ...
          performance: {
            ...
            ultimaVenda:
              vendasRevendedor.length > 0
                ? Math.max(
                    ...vendasRevendedor.map((v) => new Date(v.date).getTime()),
                  )
                : null,
          },
        };
  ...
  private searchEstoque(
    ...
    const quantidadeTotal = filteredItems.reduce(
  ...
  private generateReport(query: SearchQuery, data: any): SearchResult {
    ...
    const receitaTotal = vendas.reduce(
      (sum: number, v: Venda) => sum + v.totalValue,
      0,
    );
    const lucroTotal = vendas.reduce(
      (sum: number, v: Venda) => sum + v.netProfit,
      0,
    );
    const margemLucro =
    ...
    const relatorio = {
      ...
      lucro: lucroTotal,
      margem: margemLucro,
      revendedoresAtivos: revendedores.filter((r: Revendedor) => r.isActive)
        .length,
      valorEstoque: estoque.reduce(
        (sum: number, item: EstoqueItem) => sum + item.quantity * item.price,
        0,
      ),
    };
  ...
  private generateHelp(): SearchResult {
    ...
    return {
      ...
      summary:
        "Posso buscar informações sobre vendas, revendedores, estoque, clientes e gerar relatórios. Experimente perguntar algo!",
      actions: [
  ...
  generateNaturalResponse(result: SearchResult): string {
    ...
    const typeResponses =
      responses[result.type as keyof typeof responses] || responses.ajuda;
